...
<p>Вы можете отследить состояние своего заказа, введя его ID в поле ниже.</p>
<?php $form = ActiveForm::begin() ?>

    <?= $form->field($tracking_form, 'order_no')->label('Номер заказа') ?>
    
    <?= Html::submitButton('Отследить', ['class' => 'btn btn-default']) ?>

<?php ActiveForm::end() ?>
<br>
<?php if(isset($order_status)){
    echo '<b>Статус: </b>'.$order_status;
} ?>
